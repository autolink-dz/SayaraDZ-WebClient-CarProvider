...
describe("Modifier fabriquant actions", () => {
    let store;
    ...
    it("dispatches putFab action and get END_PUT_FABS type ", async () => {
        jest.mock('axios');
        let body={
            id:"",
            nom:"nom_test",
        ...
        await store.dispatch(putFab(body));

        const actions = store.getActions();

        expect(actions).toContainEqual({"type": "END_PUT_FABS"});
