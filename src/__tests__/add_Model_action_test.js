...
describe("add fab action", () => {
    ...
    it("dispatches addModele action and get END_GET_FABS type dispatched", async () => {
        jest.mock('axios');
        let body={
            nom:"nom_test",
            prenom:"prenom_test",
        ...
        await store.dispatch(addModele("nom", "url" , "passwordtest" ));
        await store.dispatch(getModelesList());
        ...
        //expect.assertions(3);
        expect(actions).toContainEqual({"type": "ADD_MODELE"});
